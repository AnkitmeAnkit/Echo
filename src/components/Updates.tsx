    ...
    return (
      <div className="w-full bg-canvas dark:bg-black min-h-screen pb-32">
        <div className="max-w-[1200px] mx-auto px-6 pt-12 animate-fade-in-up">
          ...
          <h1 className="text-4xl md:text-6xl font-display font-bold text-text-primary dark:text-white mb-8 tracking-tight">
            {selectedUpdate.title}
          </h1>

          {selectedUpdate.coverImage && (
            <div className="w-full aspect-[2/1] rounded-2xl overflow-hidden mb-12 border border-border-light dark:border-zinc-800">
              <img src={selectedUpdate.coverImage} alt={selectedUpdate.title} className="w-full h-full object-cover" />
            </div>
          )}

          <div className="prose prose-invert prose-lg max-w-none text-text-secondary dark:text-zinc-300">
            <p className="text-xl text-text-secondary dark:text-zinc-400 font-medium mb-8 leading-relaxed">
  ...
  return (
    <div className="w-full bg-canvas dark:bg-black min-h-screen pb-32">
      <div className="max-w-[1860px] mx-auto px-6 pt-12">
        {/* Hero Banner */}
        <div className="bg-canvas-white dark:bg-zinc-900 border border-border-light dark:border-zinc-800 rounded-[2rem] p-12 flex flex-col md:flex-row items-center justify-between gap-12 relative overflow-hidden mb-12 animate-fade-in-up">
          ...
          <div className="relative z-10 max-w-xl">
            <h1 className="text-4xl md:text-5xl font-display font-bold mb-4 text-text-primary dark:text-white">
              Updates & Insights
            </h1>
            <p className="text-text-secondary dark:text-zinc-400 text-lg leading-relaxed">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12 animate-fade-in-up" style={{ animationDelay: '0.1s' }}>
          ...
          <div className="lg:col-span-2">
            <div className="flex items-center gap-4 mb-6">
              <h2 className="text-2xl font-display font-bold text-text-primary dark:text-white">Field Notes & Research</h2>
              <Badge variant="lavender">{blogUpdates.length} updates</Badge>
            </div>

            <div className="space-y-6">
              {blogUpdates.length > 0 ? blogUpdates.map((note, i) => (
                <Card key={i} className="flex flex-col md:flex-row items-center gap-8 group cursor-pointer hover:border-brand-primary/30 transition-colors !bg-canvas-white dark:!bg-zinc-900 !border-border-light dark:!border-zinc-800" onClick={() => setSelectedUpdate(note)}>
                  <div className="flex-1">
                    <div className="flex items-center gap-3 text-[10px] font-bold text-text-tertiary dark:text-zinc-500 uppercase tracking-wider mb-3">
                    ...
                    <h3 className="text-xl text-text-primary dark:text-white font-bold mb-3 group-hover:text-brand-primary transition-colors">{note.title}</h3>
                    <p className="text-text-secondary dark:text-zinc-400 text-sm leading-relaxed mb-6">
                      {note.excerpt}
                    </p>
                  ...
                  {/* Icon Block */}
                  <div className="hidden md:flex w-24 h-24 rounded-2xl bg-canvas dark:bg-zinc-950 border border-border-light dark:border-zinc-800 text-brand-primary items-center justify-center flex-shrink-0 group-hover:scale-105 group-hover:border-brand-primary/50 transition-transform">
                    {getIconForCategory(note.category)}
                  </div>
                </Card>
              )) : (
                <div className="text-text-tertiary dark:text-zinc-500 py-8">No updates available at the moment.</div>
              )}
              
              <button className="w-full py-4 rounded-xl border border-border-light dark:border-zinc-800 text-text-secondary dark:text-zinc-400 font-bold text-sm hover:bg-canvas-white dark:bg-zinc-900 hover:text-text-primary dark:text-white transition-colors flex items-center justify-center gap-2">
          ...
          <div className="lg:col-span-1">
            <h2 className="text-2xl font-display font-bold mb-6 text-text-primary dark:text-white">The Radar</h2>
            
            <Card className="relative !bg-canvas-white dark:!bg-zinc-900 !border-border-light dark:!border-zinc-800">
              <div className="absolute top-8 bottom-8 left-[23px] w-0.5 bg-border-light dark:bg-zinc-800"></div>
                  ...
                  <div key={i} className="flex gap-4">
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-3 mb-2">
                        <span className="text-xs font-bold text-text-tertiary dark:text-zinc-500 tracking-wider">{event.date} • {event.time}</span>
                        <span className={`text-[10px] font-bold px-2 py-0.5 rounded border uppercase tracking-wider ${
                          ...
                          'text-text-secondary dark:text-zinc-400 border-zinc-400/30 bg-zinc-400/10'
                        }`}>
                          {event.status === 'SYSTEM' ? '[SYSTEM]' : event.status === 'ARCHIVE' ? '[ARCHIVE]' : event.status}
                        </span>
                      </div>
                      <p className="text-sm text-text-secondary dark:text-zinc-400 leading-relaxed mb-3">
                        {event.desc}
                      </p>
                      {event.action && (
              ...
              <div className="mt-8 pt-6 border-t border-border-light dark:border-zinc-800">
                <button className="w-full py-2 text-text-secondary dark:text-zinc-400 font-bold text-sm hover:text-text-primary dark:text-white transition-colors flex items-center justify-center gap-2">
