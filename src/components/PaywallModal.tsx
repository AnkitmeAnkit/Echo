  ...
  const modalContent = (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <motion.div
        ...
        className="relative w-full max-w-md bg-zinc-900 border border-zinc-700/50 rounded-2xl p-8 shadow-2xl z-50 overflow-hidden"
      >
        ...
        <div className="flex flex-col">
          {step === 'confirm' && (
            <div className="text-center">
              <h2 className="text-2xl font-bold text-white mb-2">{itemTitle}</h2>
              ...
              {price === 0 ? (
                <div className="flex flex-col items-center gap-6">
                  ...
                  <button
                    ...
                    className="w-full py-3.5 rounded-xl text-sm font-semibold text-white bg-brand-primary hover:bg-brand-primary/90 transition-colors"
                  >
                    Get Free Access
                  </button>
                </div>
                ...
                <div className="flex flex-col items-center gap-6">
                  <div className="text-5xl font-bold text-white tracking-tight">₹{price}</div>
                  <button
          ...
          {step === 'payment' && (
            <div>
              <div className="text-center mb-6">
                ...
                <p className="text-zinc-400 text-sm">Pay ₹{price} to acquire access</p>
              </div>

              <div className="border-2 border-dashed border-zinc-600 rounded-xl p-8 text-center mb-6 flex flex-col items-center">
                <QrCode className="w-16 h-16 text-zinc-400 mb-3" />
                <span className="text-zinc-300 font-medium">Scan QR to pay ₹{price} via PhonePe</span>
              </div>

              <div className="space-y-4 mb-8">
                <div>
                  <label className="block text-xs font-semibold text-zinc-400 uppercase tracking-wider mb-2">PhonePe / UPI ID</label>
                  <input
                    type="text"
                    placeholder="yourname@upi"
                    value={upiId}
                    onChange={(e) => setUpiId(e.target.value)}
                    className="w-full px-4 py-3 rounded-xl border border-zinc-700 bg-zinc-950 text-white placeholder:text-zinc-600 focus:outline-none focus:border-brand-primary transition-colors"
                  />
                </div>
                <div>
              ...
              <button
                ...
                className="w-full py-3.5 rounded-xl text-sm font-semibold text-white bg-brand-primary hover:bg-brand-primary/90 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
              >
                Pay ₹{price}
              </button>
            </div>
          ...
          {step === 'verifying' && (
            <div className="text-center py-12 flex flex-col items-center">
              ...
              <p className="text-zinc-400 text-sm">Please don't close this window.</p>
            </div>
          )}
          ...
          {step === 'success' && (
            <div className="text-center py-6 flex flex-col items-center">
              ...
              <div className="w-full space-y-3">
                {itemType === 'playbook' && (
                  <div className="space-y-2">
                    <button
                      ...
                      className="w-full py-3.5 rounded-xl text-sm font-semibold text-white bg-brand-primary hover:bg-brand-primary/90 transition-colors flex items-center justify-center gap-2 disabled:opacity-70"
                    >
                ...
                <button
                  onClick={handleDone}
                  className="w-full py-3.5 rounded-xl text-sm font-semibold text-zinc-300 border border-zinc-700 hover:bg-zinc-800 transition-colors"
                >
                  Done
                </button>
              </div>
